...
import { fetchSubject, fetchSubjectCount, fetchSubjectCountStudent } from "@/actions/subjects/fetch-subjects";
...
import { fetchQuizDueToday, fetchQuizDueTodayStudent } from "@/actions/quiz/fetch-quiz";
import { fetchActivitiesDueToday, fetchActivitiesDueTodayStudent } from "@/actions/activity/fetch-activity";
...
const QuickBits = () => {
  ...
  useEffect(() => {
    const fetchActiveSubjects = async () => {
      const subjectCount = await fetchSubjectCountStudent();
      const classroomId = subjectCount?.classroomData?.[0].classroom_id;

      const quizDueToday = await fetchQuizDueTodayStudent(classroomId as string);
      const activityDueToday = await fetchActivitiesDueTodayStudent(classroomId as string);
    ...
    <div className='flex flex-col gap-3'>
      ...
      {activeTab === "subjects" && (
        <div className='w-fit p-4 bg-green-300 text-green-900 dark:bg-green-600 dark:text-white rounded-md transition-colors flex flex-col gap-4'>
          <Link href='/student/subjects' className='flex gap-2 font-bold'>
            VIEW ALL SUBJECTS <ArrowRight />
          </Link>
          <div className='grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-2'>
            {subjects?.slice(0, 5).map((s) => (
              <RecentSubjectBox key={s.id} gradeAndSection={`${s.classrooms?.grade_level} - ${s.classrooms?.section}`} subject={s.name} subjectLink={`/student/subjects/${s.id}`} />
            ))}
          </div>
          {/* <RecentSubjects /> */}
        </div>
        ...
        <div className='w-fit p-4 bg-green-300 text-green-900 dark:bg-green-600 dark:text-white rounded-md transition-colors flex flex-col gap-4'>
          ...
          <div className='grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-2'>
            {quizDueTodayList?.map((s) => (
              <RecentSubjectBox key={s.id} gradeAndSection={""} subject={s.title} subjectLink={`/student/subjects/${s.subject_id}`} />
            ))}
          </div>
          {/* <RecentSubjects /> */}
        </div>
