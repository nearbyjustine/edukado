...
const DiscussionBox = ({ title, discussionId, subjectId }: { title: string; discussionId: number; subjectId: string }) => {
  ...
  return (
    <div className={cn("gap-4 items-center group", hidden ? "hidden" : "flex")}>
      <Link className='flex-1' href={`${process.env.NEXT_PUBLIC_SITE_URL}/teacher/subjects/${subjectId}/discussion/${discussionId}`}>
      ...
      <Link className='cursor-pointer hidden group-hover:inline-block hover:text-primary' href={`${process.env.NEXT_PUBLIC_SITE_URL}/teacher/subjects/${subjectId}/discussion/${discussionId}/edit`}>
